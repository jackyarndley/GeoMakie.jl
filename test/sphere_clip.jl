...
@testset "great-circle intersect (d3 oracle)" begin
    seg(a, b) = G._ixseg(
        G._cartr(a[1] * pi / 180, a[2] * pi / 180),
        G._cartr(b[1] * pi / 180, b[2] * pi / 180),
    )
    ix(a1, a2, b1, b2) = (
        x = G._gc_intersect(seg(a1, a2), seg(b1, b2));
...
@testset "resampler direction symmetry" begin
    ...
    @test all(
        ((p, q),) -> isapprox(p[1], q[1]; atol = 1e-6) && isapprox(p[2], q[2]; atol = 1e-6),
        zip(a, b),
    )
end
...
@testset "circular limb: no chord, encloses land (Bucket C)" begin
    for dest in [
        ...
        maxchord = maximum(
            hypot(sp[i][1] - sp[i-1][1], sp[i][2] - sp[i-1][2]) for i = 2:length(sp)
        )
        ...
        out = 0
        tot = 0
        for poly in G.split_geometry(GeoMakie.land(), t),
            q in GeometryBasics.coordinates(poly.exterior)

            xy = proj(q[1], q[2])
            (isfinite(xy[1]) && isfinite(xy[2])) || continue
            tot += 1
            inpoly(xy[1], xy[2]) || (out += 1)
        end
    ...
    sp =
        filter(p -> isfinite(p[1]) && isfinite(p[2]), G.boundary_points("+proj=adams_hemi"))
    cx = sum(p -> p[1], sp) / length(sp)
    cy = sum(p -> p[2], sp) / length(sp)
...
@testset "GeoAxis recipe overrides render" begin
    LAND = GeoMakie.land()
    C = GeoMakie.coastlines()
    lons = -180.0:4.0:180.0
    lats = -90.0:4.0:90.0
    ...
    for dest in ["+proj=moll +lon_0=180", "+proj=spilhaus", "+proj=igh"]
        ...
        @test_nowarn surface!(
            ...
            lons,
            lats,
            zeros(length(lons), length(lats));
            color = data,
            shading = NoShading,
        )
        # explicit per-vertex *colour* matrix (e.g. draping an image) must not force Float64(::RGBA)
        imgcol = [
            RGBA{Float32}(abs(sind(l)), abs(cosd(p)), 0.5f0, 1.0f0) for l in lons, p in lats
        ]
        @test_nowarn surface!(
            ga,
            lons,
        ...
        bigimg = [
            RGBA{Float32}(abs(sind(2l)), abs(cosd(3p)), 0.2f0, 1.0f0) for
            l in range(-180, 180; length = 200), p in range(-90, 90; length = 100)
        ]
        @test_nowarn surface!(
            ga,
            lons,
...
@testset "_resample_to_grid samples a finer image onto the z grid" begin
    A = Float64[10i + j for i = 1:4, j = 1:4]   # all distinct
...
@testset "bertin1953 native centred Hammer (Option B)" begin
    clip = G.clip_strategy(_t("+proj=bertin1953"))
    proj = G._projector(_t("+proj=bertin1953"))
    ...
    for (lo, la) in [(0.0, 0.0), (30.0, 40.0), (-100.0, -30.0), (150.0, 60.0)]
        r = fwd(deg2rad(lo), deg2rad(la))
        b = G._bertin_centred(rad2deg(r[1]), rad2deg(r[2]))
        f = proj(lo, la)
    ...
    polys, _ = G._split_geom(GeoMakie.land(), "+proj=bertin1953", _LL)
    c = clip.centred.f
    worst = 0.0
    for p in polys, k = 2:length(GeometryBasics.coordinates(p.exterior))
        pr = GeometryBasics.coordinates(p.exterior)
        worst = max(
            worst,
            hypot(c(pr[k]...)[1] - c(pr[k-1]...)[1], c(pr[k]...)[2] - c(pr[k-1]...)[2]),
        )
    end
...
@testset "Tissot pole/seam circles don't fill the whole map" begin
    ...
    function geocircle(lon, lat, radius = 500_000, n = 50)
        pts = GeometryBasics.Point2d[
            GeometryBasics.Point2d(
                reverse(Proj.geod_direct(geod, lat, lon, θ, radius)[1:2])...,
    ...
    subjects = [
        geocircle(lo, la) for (lo, la) in
        [(0.0, -90.0), (90.0, -90.0), (180.0, -81.0), (0.0, 90.0), (0.0, 0.0)]
    ]
    mapspan(p, c) = (
        xy = [
            c(q...) for
            q in GeometryBasics.coordinates(p.exterior) if all(isfinite, c(q...))
        ];
        isempty(xy) ? 0.0 :
        (maximum(first.(xy)) - minimum(first.(xy))) *
        (maximum(last.(xy)) - minimum(last.(xy)))
    )
    ...
    for dest in ["+proj=bertin1953", "+proj=moll +lon_0=180", "+proj=igh"]
        t = _t(dest)
        clip = G.clip_strategy(t)
        # bounded frame the split is drawn in (centred for the Option-B clips), to measure extent
        c =
            clip isa G.ObliqueAntimeridianClip ? clip.centred.f :
            clip isa G.AntimeridianClip ? G._projector(_t(G._centred_dest(dest))) :
            G._projector(t)
        ...
        project =
            clip isa G.ObliqueAntimeridianClip ? G._projector(clip.centred) :
            clip isa G.AntimeridianClip ? G._projector(_t(G._centred_dest(dest))) :
            G._projector(t)
