...
@testset "Unified polar support via GeoAxis" begin
    ...
    polar_cases = [
        ("north stereographic", "+proj=stere +lat_0=90 +lon_0=0 +datum=WGS84",
            (-180.0, 180.0, 60.0, 90.0)),
        ("south stereographic", "+proj=stere +lat_0=-90 +lon_0=0 +datum=WGS84",
            (-180.0, 180.0, -90.0, -60.0)),
        ("north Lambert azimuthal", "+proj=laea +lat_0=90 +lon_0=30 +datum=WGS84",
            (-180.0, 180.0, 45.0, 90.0)),
        ("north orthographic", "+proj=ortho +lat_0=90 +lon_0=0 +datum=WGS84",
        ...
        @testset "$name" begin
            ...
            for c in curves
                ...
                for i in 2:length(pts)
                    a = pts[i - 1]; b = pts[i]
                    (isfinite(a[1]) && isfinite(a[2]) && isfinite(b[1]) && isfinite(b[2])) || continue
    ...
    @testset "longitude labels around a circular polar boundary" begin
        ...
        r = 200.0
        boundary = Point2d[
            center .+ Point2d(r * cos(2π * i / 360), r * sin(2π * i / 360))
            for i in 0:360
        ]
        lons = -180.0:45.0:135.0
        curves = G.GraticuleCurve[
            G.GraticuleCurve(lon, :meridian,
                Point2d[Point2d(lon, 60), Point2d(lon, 90)],
                Point2d[center, center .+ Point2d(r * cosd(lon), r * sind(lon))])
            for lon in lons
        ]
        cands = G.place_graticule_labels(curves, lons, :meridian, boundary;
        ...
        for i in 1:length(cands), j in (i + 1):length(cands)
            a = cands[i].bbox_px; b = cands[j].bbox_px
            xov = max(0.0, min(a.origin[1] + a.widths[1], b.origin[1] + b.widths[1]) -
                max(a.origin[1], b.origin[1]))
            yov = max(0.0, min(a.origin[2] + a.widths[2], b.origin[2] + b.widths[2]) -
    ...
    @testset "latitude labels on polar parallels" begin
        ...
        parallel = G.GraticuleCurve(70.0, :parallel,
            Point2d[Point2d(-180, 70), Point2d(180, 70)],
            Point2d[
                center .+ Point2d(100 * cosd(lon), 100 * sind(lon))
                for lon in -180.0:10.0:180.0
            ])
        cands = G.place_graticule_labels([parallel], [70.0], :parallel, boundary;
            side = :left, fonts = nothing)
        @test length(cands) == 1
