...
function _gc_midpoint(lo1, la1, lo2, la2)
    x1 = cosd(la1) * cosd(lo1)
    y1 = cosd(la1) * sind(lo1)
    z1 = sind(la1)
    x2 = cosd(la2) * cosd(lo2)
    y2 = cosd(la2) * sind(lo2)
    z2 = sind(la2)
    mx = x1 + x2
    my = y1 + y2
    mz = z1 + z2
...
function _clip_faces(points, latlon, faces, proj; factor = 0.3, depth = 4)
    ...
    function mid(a, b)
        ...
        pm = proj(m[1], m[2])
        push!(ll, Point2d(m[1], m[2]))
        push!(pts, Point3d(pm[1], pm[2], 0.0))
        push!(parents, (a, b))
        idx = length(pts)
        midcache[key] = idx
        return idx
    ...
    function emit(a, b, c, d)
        ...
        (crosses(a, b) || crosses(b, c) || crosses(c, a)) ||
            (push!(out, eltype(faces)(a, b, c)); return)
...
function Makie.plot!(plot::MeshImage)
    ...
    Makie.map!(
        ...
        if (first(npoints), last(npoints)) != old_npoints[]
            ...
            rect = GeometryBasics.Tessellation(
                Rect2f(0, 0, 1, 1),
                (first(npoints), last(npoints)),
            )
            faces_buffer[] = GeometryBasics.decompose(Makie.GLTriangleFace, rect)
            uv_buffer[] =
                map(x -> Vec2f(x[1], 1.0f0 - x[2]), GeometryBasics.decompose_uv(rect))
        ...
        for (linear_ind, cartesian_ind) in enumerate(CartesianIndices((nx, ny)))
            lo = xs[cartesian_ind[1]]
            la = ys[cartesian_ind[2]]
            latlon[linear_ind] = Point2d(lo, la)
            points[linear_ind] = Makie.to_ndim(
                Point3d,
                Makie.apply_transform(tfunc, Point3d(lo, la, z_level)),
                0.0,
            )
        end
        ...
        pts, _, faces2, parents =
            _clip_faces(points, latlon, faces, _mesh_projector(tfunc, z_level))
        uvs2 = collect(Vec2f, uvs)
        for k = (length(uvs)+1):length(pts)            # UV for each inserted midpoint vertex
            a, b = parents[k]
            push!(uvs2, (uvs2[a] + uvs2[b]) / 2)
        end
