...
Many global grids store each longitude only once, so no cell bridges the last
column back to the first and `contourf`/`contour`/`surface` leave a thin gap
there. This appends a copy of the first slice along `dims` (default `1`,
longitude), with its longitudes shifted by `period`, closing the seam.

- The **vector** form takes a longitude vector and a `data` matrix whose first
  dimension is longitude; returns `(lons_c, data_c)`.
- The **matrix** form takes curvilinear `lons`, `lats`, `data` matrices and
