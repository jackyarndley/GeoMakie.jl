...
function clear_cache!(cache::AxisCache)
    empty!(cache.boundary.data)
    empty!(cache.boundary.order)
    empty!(cache.graticules.data)
    empty!(cache.graticules.order)
    empty!(cache.labels.data)
    empty!(cache.labels.order)
    return cache
end

# Bounded, thread-safe text-measurement cache shared by all axes (measurement
# depends only on the string/font/size triple).
const _TEXT_METRICS_CACHE = BoundedDict{Tuple{String,String,Float64},Rect3{Float64}}(128)
