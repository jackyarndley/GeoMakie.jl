...
function summarize(name, f; runs = 5)
    ...
    println(rpad(name, 42), "min=", lpad(round(times[1]; digits = 4), 8),
        "  median=", lpad(round(med; digits = 4), 8),
        "  mean=", lpad(round(sum(times) / length(times); digits = 4), 8),
        "  allocs=", lpad(round((sum(allocs) / length(allocs)) / 1024; digits = 1), 8), " KiB")
    return
end
...
function graticule(dest)
    ...
    GeoMakie.generate_graticule(gp, -180.0:30.0:180.0, -90.0:30.0:90.0,
...
function labels(dest)
    ...
    curves = GeoMakie.generate_graticule(gp, -180.0:30.0:180.0, -90.0:30.0:90.0,
        (-180.0, 180.0, -90.0, 90.0))
    xmin = minimum(p -> p[1], b); xmax = maximum(p -> p[1], b)
    ymin = minimum(p -> p[2], b); ymax = maximum(p -> p[2], b)
    s = 400.0 / max(xmax - xmin, ymax - ymin)
    bpix = [GeoMakie.Point2d((p[1] - xmin) * s + 50, (p[2] - ymin) * s + 50) for p in b]
    cpix = [GeoMakie.GraticuleCurve(c.coordinate, c.kind, c.geographic_geometry,
        [isnan(p[1]) ? p : GeoMakie.Point2d((p[1] - xmin) * s + 50, (p[2] - ymin) * s + 50)
            for p in c.projected_geometry]) for c in curves]
    GeoMakie.place_graticule_labels(cpix, -180.0:30.0:180.0, :meridian, bpix;
        side = :auto, fonts = nothing)
end
...
function main()
    ...
    summarize("polar stereographic graticule", () -> graticule("+proj=stere +lat_0=90 +lon_0=0"))
    summarize("global eqearth label placement", () -> labels("+proj=eqearth"))

    summarize("figure resize", () -> begin
        fig = Figure()
        ga = GeoAxis(fig[1, 1]; dest = "+proj=eqearth")
    ...
    summarize("drag pan (targetlimits update)", () -> begin
        fig = Figure()
        ga = GeoAxis(fig[1, 1]; dest = "+proj=eqearth")
        Makie.update_state_before_display!(fig)
    ...
    summarize("scroll zoom (targetlimits update)", () -> begin
        fig = Figure()
        ga = GeoAxis(fig[1, 1]; dest = "+proj=eqearth")
        Makie.update_state_before_display!(fig)
    ...
    summarize("dest change cylindrical -> polar", () -> begin
        fig = Figure()
        ga = GeoAxis(fig[1, 1]; dest = "+proj=eqearth")
    ...
    summarize("nine-panel GeoAxis figure", () -> begin
